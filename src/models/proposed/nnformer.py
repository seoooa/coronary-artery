...
class Patch_Expanding(nn.Module):
    ...
    def forward(self, x, S, H, W):
        ...
        return x
class SPADE_Patch_Expanding(nn.Module):
    def __init__(self, dim, norm_layer=nn.LayerNorm, label_nc=1):
        super().__init__()
        self.dim = dim
        ...
        # SPADE normalization
        self.spade = SPADE(dim//2, label_nc)
        
    def forward(self, x, S, H, W, segmap=None):
        B, L, C = x.shape
        assert L == H * W * S, "input feature has wrong size"
...
class BasicLayer_up(nn.Module):
    ...
    def __init__(self,
        ...
        # Use SPADE-enabled upsampling
        self.Upsample = SPADE_Patch_Expanding(dim=2*dim, norm_layer=norm_layer, label_nc=label_nc)
        
    def forward(self, x, skip, S, H, W, segmap=None):
        x_up = self.Upsample(x, S, H, W, segmap)
    ...
    def __init__(self,
        ...
        for i_layer in range(self.num_layers)[::-1]:
            ...
            layer = BasicLayer_up(
                ...
                norm_layer=norm_layer,
                upsample=SPADE_Patch_Expanding,
                label_nc=label_nc
                )
            self.layers.append(layer)
        self.num_features = [int(embed_dim * 2 ** i) for i in range(self.num_layers)]
    def forward(self, x, skips, segmap=None):
        ...
        for i in range(self.num_layers)[::-1]:
            ...
            x, S, H, W,  = layer(x,skips[i], S, H, W, segmap)
...
class nnFormer(SegmentationNetwork):
    ...
    def __init__(self, crop_size=[96,96,96],
        ...
        self.decoder=Decoder(pretrain_img_size=crop_size,embed_dim=embed_dim,window_size=window_size[::-1][1:],patch_size=patch_size,num_heads=num_heads[::-1][1:],depths=depths[::-1][1:],label_nc=label_nc)
    ...
    def forward(self, x, segmap=None):
        ...
        out=self.decoder(neck,skips,segmap)

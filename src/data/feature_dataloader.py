...
def ZeroInitializeSeg(data):
    """
    Replace segmentation with zero-initialized tensor
    """
    seg = data["seg"]  # [C, H, W, D] one-hot encoded segmentation
    
    # Create zero-initialized tensor with the same shape
...
class CoronaryArteryDataModule(pl.LightningDataModule):
    def __init__(
        ...
        guide: str = "none"  # 추가: guide 옵션 (none, segMap, dstMap)
    ):
        super().__init__()
    ...
    def prepare_data(self):
        ...
        # guide 옵션에 따라 적절한 변환 적용
        if self.guide == "none":
            # none인 경우 seg를 0으로 초기화
            transforms.append(Lambda(ZeroInitializeSeg))
        elif self.guide == "dstMap" or self.use_distance_map:
            # 거리맵 사용
        ...
        # segMap이고 거리맵을 사용하지 않을 때만 가우시안 스무딩 적용
        if self.guide == "segMap" and not self.use_distance_map:
        ...
        # guide 옵션에 따라 적절한 변환 적용 (검증용)
        if self.guide == "none":
            # none인 경우 seg를 0으로 초기화
            val_transforms.append(Lambda(ZeroInitializeSeg))
        elif self.guide == "dstMap" or self.use_distance_map:
            # 거리맵 사용
            val_transforms.append(Lambda(ConvertDistanceMap))

        # segMap이고 거리맵을 사용하지 않을 때만 가우시안 스무딩 적용 (검증용)
        if self.guide == "segMap" and not self.use_distance_map:
    ...
    def setup(self, stage=None):
        ...
        # 가이드 상태 출력
        if self.guide == "none":
            print("NO GUIDE (Zero-initialized seg tensor)")
        elif self.guide == "dstMap" or self.use_distance_map:

...
from src.data.ppe_dataloader import CoronaryArteryDataModule
...
class CoronaryArterySegmentModel(pytorch_lightning.LightningModule):
    ...
    def forward(self, x, pos):
        # Modify the forward method to include pos
        return self._model(x, pos)
    ...
    def training_step(self, batch, batch_idx):
        ...
        output = self.forward(images, poses)  # Pass pos to forward
    ...
    def validation_step(self, batch, batch_idx):
        ...
        inputs = torch.cat((images, poses), dim=1)
    ...
    def test_step(self, batch, batch_idx):
        ...
        roi_size = self.patch_size
        sw_batch_size = 4
        # Concatenate images and poses along the channel dimension
        inputs = torch.cat((images, poses), dim=1)
...
@click.option(
    "--arch_name",
    type=click.Choice(["SegResNet", "UNETR", "SwinUNETR", "PPESegResNet"]),
    default="PPESegResNet",
